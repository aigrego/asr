...
export default class NLSClient {
    ...
    private connect = (config: INLSConfig): WebSocket => {
        if (this._ws instanceof WebSocket && this._isConnected) {
            ...
            // const _ws: WebSocket = new WebSocket(config.url + "?token=" + config.token, 'audio');
    ...
    public sendText = (text?: string) => {
        ...
        this._ws.send(this.conbineCommand(text));
    }
    ...
    public conbineCommand = (text?: string) => {
        ...
        const command: INLSRequest = {
            ...
            payload: text === '_asr_stop_' ? {} : this.generateStartPayload(text),
